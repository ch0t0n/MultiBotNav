...
def plot_setting_a():
    ...
    plt.grid()
    plt.tight_layout()
    plt.savefig(f'plotting/plots/setting_a_v2_seed{args.seed}.png' if args.seed is not None else 'plotting/plots/setting_a_v2.png')
...
def plot_setting_b():
    ...
    training_logs = glob.glob(f"logs/training_best_logs/{RUN_GLOB}/tensorboard/**/events.out.tfevents.*", recursive=True)
    for log in training_logs:
        experiment_info = log.split('/')[2].split('_')
        algorithm = experiment_info[0]
        st = int(experiment_info[1][3:])
    ...
    plt.grid()
    plt.tight_layout()
    plt.savefig(f'plotting/plots/setting_b_v2_seed{args.seed}.png' if args.seed is not None else 'plotting/plots/setting_b_v2.png')
...
def plot_setting_c():
    ...
    plt.grid()
    plt.tight_layout()
    plt.savefig(f'plotting/plots/setting_c_v2_seed{args.seed}.png' if args.seed is not None else 'plotting/plots/setting_c_v2.png')
...
def plot_optuna():
    ...
    tuning_logs = glob.glob(f"logs/tuning_logs/{RUN_GLOB}/trials/trial_*/tensorboard/**/events.out.tfevents.*", recursive=True)
    ...
    plt.grid()
    plt.tight_layout()
    plt.savefig(f'plotting/plots/optuna_v2_seed{args.seed}.png' if args.seed is not None else 'plotting/plots/optuna_v2.png')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--seed', '-s', type=int, default=None, help='(v2) Filter runs by seeds.')
    ...
    RUN_GLOB = f"*_seed{args.seed}_2" if args.seed is not None else "*_2"
    
    # plot_all = not any(vars(args).values())
    plot_all = not (args.a or args.b or args.c or args.o)
