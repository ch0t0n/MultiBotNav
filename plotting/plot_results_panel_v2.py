...
def plot_panel(
    *,
    ...
    # Put the label under the plot (like (a) Setting A ...)
    ax.text(
        0.5,
        -0.22,
        subtitle,
...
def main() -> None:
    ...
    parser.add_argument("--seed", "-s", type=int, default=None, help="Filter runs by seed (omit for all seeds).")
    parser.add_argument("--version", "-v", type=int, default=2, help="Version suffix used in run names (default: 2).")
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Optional override for the output filename (inside --outdir).",
    )
    ...
    run_glob = f"*_seed{args.seed}_{args.version}" if args.seed is not None else f"*_{args.version}"
