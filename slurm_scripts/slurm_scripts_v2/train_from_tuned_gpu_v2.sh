#!/bin/bash
# Run GPU training: sbatch slurm_scripts/slurm_scripts_v2/train_from_tuned_gpu_v2.sh

#SBATCH --array=0-39           # 2 alg × 10 sets × 2 seeds = 40
#SBATCH --job-name=train_from_tuned_gpu_v2
#SBATCH --nodes=1
...
#   sbatch --export=NONE,USE_TUNED_PARAMS=True slurm_scripts/slurm_scripts_v2/train_from_tuned_gpu_v2.sh
#   USE_TUNED_PARAMS=${USE_TUNED_PARAMS:-True}

echo "ALG=$algorithm SET=$set SEED=$seed USE_TUNED_PARAMS=True DEVICE=cuda"
