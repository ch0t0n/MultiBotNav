...
def _add_tree(
    ...
    from core.visuals.tree_models import create_scenery_tree
...
def build_trees(Entity, half_w, half_h, scenery_cfg, parent=None):
    from core.visuals.tree_models import scenery_variants
    ...
    entities = []
    height_min = float(scenery_cfg.get("tree_height_min", 20.0))
    height_max = float(scenery_cfg.get("tree_height_max", 44.0))
    for _ in range(count):
        x, z = _random_pasture_point(rng, half_w, half_h, inner, outer)
        scale = rng.uniform(height_min, height_max)
        placed = _add_tree(Entity, parent, x, z, scale, rng, variants, scenery_cfg)
        if placed:
            entities.extend(placed)
    return entities


def build_horizon_trees(Entity, half_w, half_h, scenery_cfg, parent=None):
    """Distant tree ring on the horizon (replaces elliptical hill blobs)."""
    if not scenery_cfg.get("horizon_trees", True):
        return []

    from core.visuals.tree_models import scenery_variants

    count = int(scenery_cfg.get("horizon_tree_count", 52))
    dist_min = float(scenery_cfg.get("horizon_tree_distance_min", 1500.0))
    dist_max = float(scenery_cfg.get("horizon_tree_distance_max", 2700.0))
    height_min = float(scenery_cfg.get("horizon_tree_height_min", 60.0))
    height_max = float(scenery_cfg.get("horizon_tree_height_max", 100.0))
    seed = int(scenery_cfg.get("seed", 42))
    ...
    for i in range(count):
        angle = (2.0 * math.pi * i / count) + rng.uniform(-0.18, 0.18)
...
def build_rural_scenery(
    ...
    entities.extend(build_horizon_trees(Entity, half_w, half_h, scenery_cfg, parent=root))
