...
def build_meadow(Entity, cfg: dict | None = None, parent=None):
    """Vast ground plane that extends to the horizon."""
    from ursina.shaders import unlit_shader

    cfg = cfg or {}
    size = float(cfg.get("meadow_size", 14000.0))
    y = float(cfg.get("meadow_y", -0.4))
    color = tuple(cfg.get("meadow_color", (0.28, 0.44, 0.20, 1.0)))
    Entity(
        parent=parent,
        model="plane",
        scale=(size, 1, size),
...
def build_distant_hills(Entity, cfg: dict | None = None, parent=None):
    """Low hills on the horizon to sell an open agricultural landscape."""
    cfg = cfg or {}
    count = int(cfg.get("hill_count", 14))
...
def build_perimeter_fence(
    ...
    post_color = tuple(fence_cfg.get("post_color", fence_cfg["color"]))
    rail_color = tuple(fence_cfg.get("rail_color", fence_cfg["color"]))
