...
# with textures assigned explicitly (see scripts/split_tree_obj.py).
...
def goal_variant(goal_cfg: dict) -> str:
    ...
    if not found:
        raise FileNotFoundError(
            "No tree models found. Run: python scripts/download_assets.py"
        )
    return found[0]
...
def pick_variant(variants: list[str], index: int, rng: random.Random | None = None) -> str:
    if not variants:
        raise FileNotFoundError(
            "No tree models found. Run: python scripts/download_assets.py"
        )
