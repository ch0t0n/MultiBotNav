...
def fix_path(rel_path):    
    # Convert a Jupyter-style relative path into an absolute path    
    try:
        base = os.path.dirname(os.path.abspath(__file__))  # .py file
    except NameError:
        base = os.getcwd()  # Jupyter fallback
    return os.path.abspath(os.path.join(base, rel_path))

# Path to the consolidated JSON config and the environment key to load
max_steps   = 1000
env_key     = 'env6'   # change to 'env2' … 'env10' to simulate other environments
num_robots  = None     # set to 2–5 to override; None = infer from checkpoint
wheeled_json_path = fix_path(r"..\exp_sets\wheeled\wheeled_configs.json")
weights_path = fix_path(r"..\trained_models\wheeled\best_model_env6_stage2_robust_wind\best_model.zip")

def _infer_num_robots(observation_space):
    """Infer robot count from full-mode observation shape (5N+1)."""
...
def _resolve_num_robots(weights_path, num_robots):
    if num_robots is not None:
        return num_robots
    checkpoint = CrossQ.load(weights_path)
    inferred = _infer_num_robots(checkpoint.observation_space)
    del checkpoint
    return inferred
...
if __name__ == "__main__":
    ...
    num_robots = _resolve_num_robots(weights_path, num_robots)
    print(f"Using {env_key} with {num_robots} robots")

    # Match v6_wheeled training: slice to the requested robot count.
    env_params["NUM_ROBOTS"]         = num_robots
    env_params["ROBOT_INIT_CONFIGS"]   = env_params["ROBOT_INIT_CONFIGS"][:num_robots]

    # Stage-2 training used wind uncertainty + wind domain randomisation.
    env = MultiWheeled(
    ...
    # Load trained network (env required so obs/action spaces match the checkpoint).
    model = CrossQ.load(weights_path, env=env)
    env.metadata['render_fps'] = 30
    obs, info = env.reset()
    env.render()
    pygame.image.save(env.screen, f"wheeled_init_{env_key}.jpg")
