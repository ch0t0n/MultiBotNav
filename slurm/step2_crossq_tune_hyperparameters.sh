#!/bin/bash
# ============================================================
# Step 2 — Hyperparameter tuning (Optuna, fully parallelised), CrossQ (GPU)
#
...
#
# Grid: 2 algorithms × 50 trials = 100 jobs per robot type
#   sbatch --array=0-99 step2_crossq_tune_hyperparameters.sh uav
#   sbatch --array=0-99 step2_crossq_tune_hyperparameters.sh wheeled#
# Index layout:
#   alg_idx   = index // 50        (0–1)
...
#SBATCH --array=0-99
#SBATCH --job-name=s2_crossq_tune
#SBATCH --output=logs/slurm_outputs/s2_crossq_tune/%x_%A_%a.out
#SBATCH --error=logs/slurm_errors/s2_crossq_tune/%x_%A_%a.err
#SBATCH --nodes=1
...
echo "S2-CrossQ-tune | robot_type=${ROBOT_TYPE} | alg=${algorithm} | trial=${trial_idx} | device=${device} | job=${SLURM_ARRAY_TASK_ID}"
