...
def main():
    ...
    # Allow up to 4 concurrent processes per GPU for GPU-bound workloads.
    max_gpu_procs = max(num_gpus * 4, 4) if num_gpus > 0 else max_cpu_procs

    def run_batches(jobs, batch_size, label):
        total = len(jobs)
        print(f"\n--- {label}: {total} jobs, batch_size={batch_size} ---")
        for start in range(0, total, batch_size):
            batch = jobs[start : start + batch_size]
            procs = [mp.Process(target=train_single_env, args=job) for job in batch]
            for p in procs:
                p.start()
            for p in procs:
                p.join()
            done = min(start + batch_size, total)
            print(f"  Completed {done}/{total} {label} jobs")

    run_batches(cpu_jobs, max_cpu_procs, "CPU (A2C, ARS, PPO, TRPO)")
    run_batches(gpu_jobs, max_gpu_procs, "GPU (CrossQ, TQC)")
