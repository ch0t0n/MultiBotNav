    ...
    def step(self, actions):
        ...
        for i in range(self.num_robots):
            ...
            if self.reward_ablation != "no_path":
                rewards -= 0.1 * (ax ** 2 + ay ** 2)
                rewards -= 0.3 * float(np.linalg.norm(self.robot_velocities[i]))

            pos_key = tuple(np.round(self.robot_positions[i], 1))
            rewards -= 100 if pos_key in self.visited else 10
        ...
        if self.reward_ablation != "no_path":
            rewards -= 1.0 * step_path
            rewards -= 2.0

        # Distance shaping to nearest unvisited target (always active)
