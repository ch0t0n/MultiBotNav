    ...
    def step(self, actions):
        ...
        for i in range(self.num_robots):
            ...
            if pos_key in self.visited:
                rewards -= 5        # revisit deterrent (was -100; 20× reduction prevents runaway)
            else:
                rewards += 0.5      # exploration bonus (was -10; flipped — penalising new cells
            ...
            for inf_loc in list(self.infected_locations):
                if np.linalg.norm(self.robot_positions[i] - np.array(inf_loc)) <= self.infected_size:
                    ...
                    rewards += 200      # was +10,000 — 50× reduction
        ...
        if len(self.infected_locations) == 0:
            if self.reward_ablation != "no_term":
                rewards += 1000      # FIX: was +100,000 — matches reference env success bonus
            term_cond  = "visited_all"
            terminated = True

        if self.num_robots > 1 and compute_min_dist(self.robot_positions) < self.robot_size:
            if self.reward_ablation != "no_term":
                rewards -= 1000     # FIX: was -100,000 — 2× success bonus, matches reference env
            term_cond  = "collision"
            terminated = True
