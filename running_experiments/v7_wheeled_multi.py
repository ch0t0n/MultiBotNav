...
# Algorithm Registry
# ================================
CPU_ALGS = frozenset({"A2C", "ARS", "PPO", "TRPO"})
GPU_ALGS = frozenset({"CrossQ", "TQC"})

_ALG_MAP = {"A2C": A2C, "ARS": ARS, "PPO": PPO, "TRPO": TRPO, "CrossQ": CrossQ, "TQC": TQC}

def build_model(alg_name, policy, vec_env, device, seed):
    """Instantiate the correct SB3/sb3_contrib model with default hyperparameters."""
    cls = _ALG_MAP[alg_name]
    return cls(policy, vec_env, verbose=0, device=device, seed=seed)


# ================================
# Training Worker (one env per process)
# ================================
def train_single_env(env_id, config, seed, alg_name, n_robots):
    ...
    # ── Device and parallel-env count ────────────────────────────────────
    if alg_name in CPU_ALGS:
        device = "cpu"
        n_envs = config["num_envs_cpu"]
    else:
        device = gpu_ids[(env_id + n_robots + seed) % len(gpu_ids)] if gpu_ids else "cpu"
        n_envs = config["num_envs_gpu"]

    # ── Build env_params with the requested robot count ──────────────────
    raw_params = json_dict[f"set{env_id}"]
    env_params = dict(raw_params)
    env_params["NUM_ROBOTS"]         = n_robots
    env_params["ROBOT_INIT_CONFIGS"] = list(raw_params["ROBOT_INIT_CONFIGS"])[:n_robots]

    gym_id = "MultiWheeled-v0"

    # ── 2-stage curriculum ────────────────────────────────────────────────
    # Stage 1 (70 %): deterministic, no domain randomisation → fast initial discovery
    # Stage 2 (30 %): wind uncertainty + wind DR              → policy robustness
    ...
    kwargs_s2 = {
        ...
        "dr_mode":          "wind",
    } if stage2_steps > 0 else None

    stage_specs = [("stage1", kwargs_s1, stage1_steps)]
    if kwargs_s2:
        stage_specs.append(("stage2", kwargs_s2, stage2_steps))

    run_tag = f"{alg_name}_N{n_robots}_seed{seed}_env{env_id}"
    run_dir = os.path.join(log_path, f"{alg_name}_N{n_robots}_seed{seed}", f"env{env_id}")
    os.makedirs(run_dir, exist_ok=True)

    print(f"[START] {run_tag} on {device}")

    vec_env = make_vec_env(gym_id, env_kwargs=kwargs_s1, n_envs=n_envs, seed=seed, monitor_dir=None)
    logger  = configure(os.path.join(run_dir, "tb_logs"), ["stdout", "log", "csv", "tensorboard"])
    model   = build_model(alg_name, "MlpPolicy", vec_env, device, seed)
    model.set_logger(logger)

    for stage_idx, (stage_name, kwargs, steps) in enumerate(stage_specs):
        if steps <= 0:
            continue
        if stage_idx > 0:
            vec_env.close()
            vec_env = make_vec_env(gym_id, env_kwargs=kwargs, n_envs=n_envs, seed=seed, monitor_dir=None)
            model.set_env(vec_env)

        eval_env = make_vec_env(gym_id, env_kwargs=kwargs, n_envs=1, seed=seed, monitor_dir=None)
    ...
    model.save(os.path.join(run_dir, f"{run_tag}_final"))
    vec_env.close()
...
def main():
    ...
    version       = "7"
    robot_type    = "wheeled"
    # seeds         = [42, 0, 123]          # 3 seeds
    seeds = [42]
    n_robots_list = [2, 3, 4, 5]          # robot counts for the table
    algorithms    = ["A2C", "ARS", "PPO", "TRPO", "CrossQ", "TQC"]

    num_gpus = torch.cuda.device_count()
    gpu_ids  = [f"cuda:{i}" for i in range(num_gpus)]

    config_base = {
        "robot_type":    robot_type,
        "gpu_ids":       gpu_ids,
        "time_steps":    int(2e6),
        "num_envs_cpu":  8,   # parallel envs per CPU-algorithm process
        "num_envs_gpu":  8,   # parallel envs per GPU-algorithm process
        "max_steps":     1000,
    }
    ...
    log_path = os.path.join(".", "logs", f"{date}_v{version}_{robot_type}_default_hp")
    ...
    # ── BUILD JOB LISTS ────────────────────────────────────────────────────
    # Jobs are (env_id, config, seed, alg_name, n_robots) — one per process.
    # CPU and GPU jobs are run in separate passes to avoid resource contention.
    cpu_jobs, gpu_jobs = [], []
    for seed in seeds:
        for n_robots in n_robots_list:
            for alg in algorithms:
                for env_id in env_ids:
                    job = (env_id, config, seed, alg, n_robots)
                    (cpu_jobs if alg in CPU_ALGS else gpu_jobs).append(job)

    total_jobs    = len(cpu_jobs) + len(gpu_jobs)
    max_cpu_procs = mp.cpu_count()
    # 4 concurrent processes per GPU; fall back to cpu_count when no GPUs present.
    max_gpu_procs = max(num_gpus * 4, 4) if num_gpus > 0 else max_cpu_procs

    print(f"\n=== v{version} wheeled default-HP sweep: {total_jobs} total jobs ===")
    print(f"    seeds={seeds}  robots={n_robots_list}  algs={algorithms}")
    print(f"    GPUs detected: {num_gpus}  ({gpu_ids})")
    print(f"    CPU pool: {max_cpu_procs} workers | GPU pool: {max_gpu_procs} workers")
    print(f"    Both pools drain concurrently — no idle waiting.\n")

    def _drain_pool(jobs, n_workers, label):
        """
        Submit all jobs to a fixed-size pool.  Workers pick up the next job the
        moment they finish the current one — no batch-level synchronisation.
        """
        if not jobs:
            return
        print(f"[POOL] {label}: {len(jobs)} jobs → {n_workers} workers (started)")
        with mp.Pool(processes=n_workers) as pool:
            pool.starmap(train_single_env, jobs)
        print(f"[POOL] {label}: all {len(jobs)} jobs complete")

    # Launch CPU and GPU pools in separate threads so they drain simultaneously.
    cpu_thread = threading.Thread(
        target=_drain_pool,
        args=(cpu_jobs, max_cpu_procs, "CPU (A2C, ARS, PPO, TRPO)"),
        daemon=True,
    )
    gpu_thread = threading.Thread(
        target=_drain_pool,
        args=(gpu_jobs, max_gpu_procs, "GPU (CrossQ, TQC)"),
        daemon=True,
    )
