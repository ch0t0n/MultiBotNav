...
def train_single_env(env_id, config, seed=42):
    ...
    nr_tag   = f"_nr{num_robots}" if robot_type == "wheeled" else ""
    run_name = f"env{env_id}_seed{seed}_{robot_type}{nr_tag}"
    ...
    if robot_type == "uav":
        ...
        # Build an env_params copy with the requested robot count.
        # The JSON always stores 5 configs; slice to the first num_robots entries.
        base_params = dict(json_dict[set_key])
        base_params["NUM_ROBOTS"]        = num_robots
        base_params["ROBOT_INIT_CONFIGS"] = base_params["ROBOT_INIT_CONFIGS"][:num_robots]
    ...
    model.save(os.path.join(log_path, f"env{env_id}_CrossQ_{robot_type}{nr_tag}"))

    vec_env.close()
    print(f"[DONE]  Training env {env_id}, seed {seed}, robot_type {robot_type}{nr_tag}")
...
def main():
    ...
    config_base = {
        ...
        "time_steps": int(3e6),
        "num_envs":   8,
        "num_robots": 3,           # only used by MultiUAV; overridden per iteration for MultiWheeled
        "max_steps":  1000,
    }
    ...
    # For wheeled experiments, vary the number of robots from 2 to 5.
    # UAV keeps its fixed num_robots from config_base.
    robot_counts = list(range(2, 6)) if robot_type == "wheeled" else [config_base["num_robots"]]

    # ── MULTI-SEED LOOP ────────────────────────────────────────────────────
    for seed in seeds:
        for nr in robot_counts:
            date     = datetime.now().strftime('%b%d_%H')
            nr_tag   = f"_nr{nr}" if robot_type == "wheeled" else ""
            log_path = os.path.join(".", "logs", f"{date}_v{version}_{robot_type}{nr_tag}_seed{seed}")
            os.makedirs(log_path, exist_ok=True)

            print(f"\n========== SEED {seed} | robot_type={robot_type} | num_robots={nr} ==========\n")

            config              = config_base.copy()
            config["num_robots"]  = nr
